...
export function useNumber(options?: NumberFormatterOptions) {
  ...
  const decimal = (precision: number) => {
    return {
      style: 'decimal', minimumFractionDigits: 0, maximumFractionDigits: precision,
    };
  };
  const percent = (precision: number) => {
    return {
      style: 'percent', minimumFractionDigits: 0, maximumFractionDigits: precision,
    };
  };
  ...
  function roundTo(inputValue: string, precision: number, option: RoundOption = 'round'): number | null {
